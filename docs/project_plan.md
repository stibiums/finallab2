...
| hard-layout partner robustness | mixed | `unident_s` is robust, `random1` remains brittle under held-out partners, and `random0` is asymmetric. Two-partner and three-partner `random1` diversity runs improve parts of the seen-partner pool; a mixed fixed + learned partner run is worse; partner-id conditioning improves four-partner avg/min but still does not solve robustness. |
...
Status: initial hard-layout matrices completed; two-partner and three-partner `random1` partner-aware training runs are also completed, but true held-out robustness remains unresolved.
...
| `random1` | 5.20 to 5.80 soups | 0.00 to 1.10 soups across four fixed partners; `partner_diversity_random1_three_partners` reaches 0.65 min over its three seen partners and 1.00 with held-out seed73; the mixed fixed + learned partner run averages only 0.69; partner-id conditioning improves to 2.34 average and 0.80 minimum over four known partners | Strong partner brittleness despite good self-play. Partner conditioning helps but is not yet robust. |
| `unident_s` | 12.60 to 12.70 soups | 12.60 to 12.65 soups | Robust across two independent seeds. |

Decision: partner robustness must be reported per layout. `unident_s` can be described as robust across the tested seeds, but `random1` should be described as a self-play specialist rather than a generally compatible teammate. The `partner_diversity_random1` and `partner_diversity_random1_three_partners` results are useful evidence that partner-aware training helps some seen partner styles, not evidence that the router has a robust `random1` teammate. The mixed fixed + learned partner run is a negative result. The partner-id conditioned run is the strongest `random1` partner-aware diagnostic so far, but it only improves known-partner compatibility; true robustness still needs unknown-partner evaluation and a stronger MARL formulation.
...
4. If more experiment time remains, extend the `random1` partner-conditioned result: evaluate unknown partners, add stronger conditioning/regularization, or try MAPPO/HAPPO/HARL-style heterogeneous-agent training. The first partner-id conditioned run improves four-known-partner avg/min but does not solve robustness.
